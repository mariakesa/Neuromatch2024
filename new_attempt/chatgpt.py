...
class Agent:
    ...
    def learn(self):
        ...
        hidden_tensor = torch.stack(
            [h.clone().detach().to(torch.float32) for h in batch.hidden]
        ).unsqueeze(0).to(self.device)
        ...
        next_hidden_tensor = torch.stack(
            [h.clone().detach().to(torch.float32) for h in batch.next_hidden]
        ).unsqueeze(0).to(self.device)
        ...
        target_q_values = rewards_tensor + self.gamma * \
            next_q_values * (1 - dones_tensor)
        target_q_values = target_q_values.squeeze(0)  # Shape [64]
