...
class Agent:
    def __init__(self, state_size, action_size, hidden_size, capacity, batch_size, lr, gamma):
        ...
        self.dqn_network = MyDQN(cnn_feature_size=10,
                                 hidden_size=64, action_size=action_size).to(device='cuda')
    ...
    def select_action(self, state, hidden):
        ...
        if rand > self.epsilon:
            with torch.no_grad():
                hidden = self.q_network(
                    state.reshape(1, 1, -1), hidden)
                q_values = self.dqn_network(state.reshape(1, 1, -1), hidden)
    ...
    def learn(self):
        ...
        current_q_values = self.dqn_network(states_tensor, hidden_tensor)
